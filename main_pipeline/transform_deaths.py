...
path = os.path.join(os.path.dirname(__file__), 'temp', 'cleaned_deaths.csv')
...
def main():
    logger.info("Reading Raw Death data")
    df = read_csv_temp(os.path.join(os.path.join(os.path.dirname(__file__)), 'temp', 'deaths.csv'))
    logger.info("Reading raw death data Completed")
    logger.info("Transforming raw deaths data")
    df = transform.parse_dates(df, ['date'])
    df = transform.create_country_col(df)
    df = transform.create_unique_from_columns(df)
    df.columns = cols
    logger.info("Transformation Completed (raw deaths)")
    ...
    logger.info("Saving transformed Deaths Completed")
