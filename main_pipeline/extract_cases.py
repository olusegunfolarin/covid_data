...
# Set the url to the data source
...
path = os.path.join(os.path.dirname(__file__), 'temp', 'cases.csv')

# Extract the data   
def main():
    logger.info('Starting Cases Extraction')
    csv_data = get_csv_from_requests(url)
    logger.info('Finished Extracting Cases')
    logger.info('Staging extracted data')
    save_to_csv_temp(csv_data, path)
    logger.info('Staging complete for Cases')
