...
path = os.path.join(os.path.dirname(__file__), 'temp', 'cleaned_vaccinations.csv')
...
def main():
    logger.info("Reading Raw vaccinations data")
    df = read_csv_temp(os.path.join(os.path.join(os.path.dirname(__file__)), 'temp', 'vaccinations.csv'))
    logger.info("Reading raw vaccinations data Completed")
    logger.info("Transforming raw vaccinations data")
    df = transform.parse_dates(df, ['date'])
    df = transform.create_country_col(df)
    df = transform.create_unique_from_columns(df)
    df.columns = cols
    logger.info("Transformation Completed (raw vaccinations)")
